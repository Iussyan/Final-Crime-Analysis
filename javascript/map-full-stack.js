...
L.tileLayer(
  "https://api.maptiler.com/maps/openstreetmap/256/{z}/{x}/{y}.jpg?key=kmDDNhXUXCZxn5Sfpecq",
  {
    maxZoom: 19,
    attribution:
        ...
        onEachFeature: function (feature, layer) {
          ...
          layer.bindPopup(
            () => `
                      ...
                      <button class="btn btn-sm btn-primary mt-2" onclick="fetchIncidentDetails(${props.streetId})" data-bs-toggle="modal" data-bs-target="#incidentModal">
                      <i class="bi bi-search me-1"></i> View Incidents
                      </button>
                      </div>`
          );
    ...
    .then((data) => {
      const turfPoints = data.features.map((f) => {
        ...
        return turf.point([mid[0], mid[1]], {
          ...
          streetId: f.properties.streetId || "Unknown Street ID",
      ...
      const clusterWeights = Object.values(clustersMap).map((cluster) =>
        cluster.reduce((sum, pt) => sum + (pt.properties.weight || 1), 0)
      );
      const maxWeight = Math.max(...clusterWeights);

      // Build heat points with normalized weights
      Object.values(clustersMap).forEach((cluster, index) => {
        ...
        heatPoints.push([avgLat, avgLng, normalizedWeight]);
        ...
        const uniqueIds = [
          ...new Set(cluster.map((p) => p.properties.streetId)),
        ];
        const categories = [
        ...
        const popupContent = `
        <div style="min-width:200px;">
        <h4 style="margin-bottom:4px;">Cluster #${index + 1}</h4><br>
        <strong>Total Crimes:</strong> ${totalWeight}<br>
        <strong>Streets:</strong> ${uniqueStreets.join(", ")}<br>
        <strong>Categories:</strong> ${categories.join(", ")}<br><br>
        <button class="btn btn-sm btn-primary mt-2" onclick='openIncidentModal(${JSON.stringify(
          uniqueIds
        )})'>
        <i class="bi bi-search me-1"></i> View Reports
        </button>
        </div>`;
      ...
      setTimeout(() => {
        document.querySelectorAll(".cluster-btn").forEach((btn) => {
          btn.addEventListener("click", function (e) {
            ...
            console.log("Cluster button clicked:", clusterId);
            // Placeholder action
            alert(`You clicked Cluster #${parseInt(clusterId) + 1}`);
          });
        });
      }, 300); // Delay to ensure popups are in DOM
    });
}

function openIncidentModal(streetIds) {
  const modal = new bootstrap.Modal(document.getElementById("incidentModalC"));
  modal.show();

  const modalBody = document.getElementById("incidentModalBodyC");
  modalBody.innerHTML = `
  ...
  fetch("../secure/get_cluster_incidents.php", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ streetIds }), // ✅ FIXED: key should be streetIds
  })
    .then((res) => res.json())
    .then((response) => {
      if (
        ...
        modalBody.innerHTML = `
            <div class="alert alert-danger text-center">
              <i class="bi bi-info-circle me-2"></i> No incidents found for these streets.
            </div>`;
        return;
      }

      const incidentHtml = response.data
        .map(
          (i) => `
          <div class="card mb-3 shadow-sm">
            <div class="card-body text-dark">
              <h6 class="card-title mb-1">${i.category} - ${i.crimeType}</h6>
              <p class="card-text small mb-1"><strong>Description:</strong> ${i.crimeDescription}</p>
              <p class="card-text small mb-0">
                <strong>Date:</strong> ${i.date} | 
                <strong>Time:</strong> ${i.time} | 
                <strong>Street:</strong> ${i.street}
              </p>
            </div>
          </div>`
        )
      ...
      modalBody.innerHTML = incidentHtml;
    })
    .catch((err) => {
      console.error("Incident fetch error:", err);
      modalBody.innerHTML = `
          <div class="alert alert-danger text-center">
            <i class="bi bi-exclamation-triangle-fill me-2"></i> Error loading incident data.
          </div>`;
    });
}
...
fullscreenButton.onAdd = function () {
  const div = L.DomUtil.create("div", "fullscreen-button");
  div.innerHTML = `<button id="toggle-fullscreen" title="Fullscreen Map" class="map-button">
    <img src="../src/icons/fit-screen.svg" alt="Fullscreen" style="width:20px;height:25px;">
  </button>`;
  return div;
};
...
// Add the actual fullscreen logic
...
toggleMarkerButton.onAdd = function () {
  ...
  div.innerHTML = `<button id="toggle-marker" title="Toggle Marker" class="map-button">
      ...
      setTimeout(() => {
        ...
        domtoimage.toPng(container)
          ...
          .catch(function (error) {
            ...
            alert('Failed to capture the map snapshot.');
          })
          .finally(() => {
...
L.control.snapshot = function(opts) {
  return new L.Control.Snapshot(opts);
};
L.control.snapshot({ position: 'topleft' }).addTo(map);

function fetchIncidentDetails(streetId) {
  const modalBody = document.getElementById("incidentModalBodyP");
  modalBody.innerHTML = `
      <div class="text-center text-muted">
  ...
  fetch(`../secure/get_incidents_by_streets.php?streetId=${streetId}`)
    ...
    .then((data) => {
      if (!data.length) {
        modalBody.innerHTML =
          "<p class='text-center text-danger'>No incidents found for this street.</p>";
        return;
      }
      ...
      <div class="border rounded-3 p-3 mb-3 bg-light-subtle shadow-sm">
        <h6 class="mb-3 fw-semibold text-primary">Incident #${i + 1}</h6>

        <ul class="list-unstyled mb-0 small text-dark">
          <li><strong>ID:</strong> ${incident.IncidentId}</li>
          <li><strong>Address:</strong> ${incident.Address || "Unknown"}</li>
          <li><strong>Street:</strong> ${incident.Street || "N/A"}</li>
          <li><strong>Street ID:</strong> ${incident.StreetId}</li>
          <hr>
          <li><strong>Category:</strong> ${incident.Category}</li>
          <li><strong>Crime Type:</strong> ${incident["Crime Type"]}</li>
          <li><strong>Description:</strong> ${
            incident["Crime Description"]
          }</li>
          <hr>
          <li><strong>Witness Name:</strong> ${
            incident["Witness Name"] || "Anonymous"
          }</li>
          <li><strong>Age:</strong> ${incident["Witness Age"] || "N/A"}</li>
          <li><strong>Sex:</strong> ${incident["Witness Sex"] || "N/A"}</li>
          <li><strong>Contact:</strong> ${
            incident["Witness Contact"] || "N/A"
          }</li>
        </ul>        
              <div class="d-flex gap-2 m-t-5">
                <button class="btn btn-sm btn-outline-primary export-csv" data-idx="${i}">Export CSV</button>
                <button class="btn btn-sm btn-outline-danger export-pdf" data-idx="${i}">Export PDF</button>
              </div>
      </div>`;
        })
        .join("");
